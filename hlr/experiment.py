...
class SpacedRepetitionModel(object):
    ...
    def dump_detailed_predictions(self, fname, testset):
        with open(fname, 'w', encoding='utf-8', newline='') as f:
            ...
            for inst in testset:
                pp, hh = self.predict(inst)
                base_str = f"\t{pp:.4f}\t{inst.h:.4f}\t{hh:.4f}\t{inst.lang}\t{inst.uid}\t{inst.ts}\t{inst.lexeme}\n"
                
                # Multiply the string instead of looping!
                if inst.right > 0:
                    f.write(f"1.0{base_str}" * inst.right)
                if inst.wrong > 0:
                    f.write(f"0.0{base_str}" * inst.wrong)
